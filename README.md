...
Домен вдохновлён двумя годами работы в подписке Яндекс Плюса — grace period, retention-офферы, начисление баллов это не выдуманные требования, а то, с чем реально приходилось работать. Дальнейшее развитие — [Sublite Distributed](#что-дальше): та же доменная модель, но биллинг вынесен в отдельный event-driven сервис (Kafka, Outbox, Saga, Kubernetes).
...
**Sublite Distributed** — тот же домен, но биллинг выделен в отдельный event-driven сервис: Kafka, Outbox pattern, Saga, Kubernetes. История для собеседования: "сначала сделал монолит, потом выделил биллинг в отдельный сервис — вот с какими проблемами столкнулся и как их решал".
